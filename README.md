...
Visão Computacional: Técnicas de pré-processamento e binarização de imagens.

NLP (Processamento de Linguagem Natural): Orquestração de prompts e estratégias de chunking.

Colaboração via Git: Fluxo de trabalho baseado em Branches, Pull Requests e Code Reviews rigorosos.
...
Visão: OpenCV, Tesseract, EasyOCR (em avaliação).

LLMs: APIs do Google Gemini, OpenAI ou modelos locais via LangChain.
...
Abrimos um Pull Request (PR) para comparar resultados (performance, precisão e limpeza de código).
...
Por questões de performance e boas práticas de Git, a pasta data/ contendo as imagens de treino/teste está listada no .gitignore. Solicite o acesso ao dataset compartilhado com os membros do grupo.
